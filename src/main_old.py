...
from .utils.config import DOCUMENT_NON_EXISTING, DOCUMENT_END
class VoiceAssistant:
    ...
    def process_buffered_text(self, lang_code="ru"):
        ...
        context_qa = self.document_loader.search_document(combined_text)
        print(context_qa)
        segments = []
        if not context_qa:
            ...
            messages = self.document_loader.construct_prompt_message(combined_text, context_qa)
            gpt_response = self.gpt_handler.generate_response_from_messages(messages)
            self.logger.info(f"GPT: {gpt_response}")
